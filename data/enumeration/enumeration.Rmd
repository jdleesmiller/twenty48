...
```{r}
plotLayerPartsSummary <- function (layerParts, xBreaks) {
  ggplot(
    transform(
      layerParts,
    ...
    ggtitle('Number of States per Sum Layer') +
...
```{r}
local({
  g <- plotLayerPartsSummary(partialParts, 32 * seq(1, 12))
...
```{r}
plotLayerPartsLog <- function (layerParts, xBreaks) {
  ...
  ggplot(
    ...
    scale_y_continuous(
      labels = function (n) format(n, scientific = FALSE, big.mark = ','),
      trans = 'log10') +
    ggtitle('Number of States per Sum Layer and Max Tile Value') +
    xlab('Sum of Tiles in State') +
